...
SYMBOLS = {s.name: s for s in [
    two_pin("R2", "R"), two_pin("C2", "C"), two_pin("LED2", "D"),
    conn4(), mcu8(), mcu48(),
]}
...
class Fixture:
    ...
    def add(self, ref: str, sym: str, value: str, x: float, y: float, **nets: str) -> None:
        ...
        self.components.append(Component(ref, sym, value, x, y,
                                         {k.lstrip("p"): v for k, v in nets.items()}))
    ...
    def ground_truth(self) -> dict:
        ...
        return {
            "components": {c.ref: {"value": c.value, "lib": f"FIXLIB:{c.sym}",
                                   "x": c.x, "y": c.y} for c in self.components},
            "nets": {k: sorted(v) for k, v in sorted(nets.items())},
            "unconnected_pins": sorted(unconnected),
            "counts": {"components": len(self.components), "nets": len(nets),
...
def build_001() -> Fixture:
    ...
    f.add("U1", "MCU8", "MCU-8P", 127.0, 63.5,
          p1="3V3", p2="GND", p3="SDA", p4="SCL", p5="", p6="LED_A", p7="GND2X", p8="GND2X")
...
def build_002() -> Fixture:
    ...
    for i in range(1, 9):                   # 8 resistencias serie + 8 LEDs
        f.add(f"R{i + 2}", "R2", "330", 190.5 + i * 10.16, 101.6,
              p1=f"LED{i}", p2=f"LEDK{i}")
        f.add(f"D{i}", "LED2", "LED", 190.5 + i * 10.16, 127.0,
              p1=f"LEDK{i}", p2="GND")
...
def build_003() -> Fixture:
    ...
    for b in range(10):
        bx = 63.5 + (b % 5) * 76.2
        by = 63.5 + (b // 5) * 127.0
        f.add(f"J{b + 1}", "CONN4", f"BLK{b + 1}", bx, by,
              p1=f"B{b + 1}_A", p2=f"B{b + 1}_B", p3=f"B{b + 1}_C", p4="GND")
        for r in range(14):
            rx = bx + 5.08 + (r % 7) * 2.54
            ry = by - 12.7 + (r // 7) * 25.4
            net_in = [f"B{b + 1}_A", f"B{b + 1}_B", f"B{b + 1}_C"][r % 3]
            f.add(f"R{b * 14 + r + 1}", "R2", "1k", rx, ry,
                  p1=net_in, p2="GND")
    return f
...
def main(out_root: Path) -> None:
    for fx in (build_001(), build_002(), build_003()):
        ...
        print(f"{fx.name}: {gt['counts']['components']} comp, "
