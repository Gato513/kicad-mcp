...
| `delete_tracks_bulk` | Borra tracks/vías por filtro (`net`/`bbox`/`layer`) en un round-trip; `dry_run` lista sin mutar (sesión 19d) — devuelve JSON `{tracks_deleted, vias_deleted, snap_id, zones_refilled}`. Si el borrado toca ≥1 zona de cobre, sesión 34a-fix-1 (D-23.2, ADR-0012) persiste a disco el refill+enforce_hole_clearance del vivo antes de retornar — mismo contrato que `add_zone`/`fill_zones` (sesión 27) | `net?`, `bbox?`, `layer?`, `include_vias?=true`, `dry_run?=false`, `base_snap?` | JSON | `NET_NOT_FOUND`, `INVALID_PARAMS`, `PROJECT_NOT_FOUND`, `KICAD_NOT_RUNNING`, `KICAD_TIMEOUT`, `KICAD_RESTARTED`, `SNAPSHOT_STALE`, `EXTERNAL_EDIT_DETECTED`, `POST_ZONE_PERSIST_FAILED` |
...
| `add_zone` | Crea una zona de cobre conectada a un net en una capa (sesión 19, P4.1) — devuelve JSON `{zone_id, filled, area_mm2, snap_id}`. Con `fill=true`, sesión 27 (D-23.2, ADR-0012) persiste a disco el refill+enforce_hole_clearance del vivo antes de retornar | `net`, `layer`, `bbox?`, `polygon?`, `priority?=0`, `fill?=true`, `base_snap?` | JSON | `NET_NOT_FOUND`, `INVALID_ZONE_GEOMETRY`, `INVALID_PARAMS`, `PROJECT_NOT_FOUND`, `KICAD_NOT_RUNNING`, `KICAD_TIMEOUT`, `KICAD_RESTARTED`, `SNAPSHOT_STALE`, `EXTERNAL_EDIT_DETECTED`, `POST_ZONE_PERSIST_FAILED` |
...
**Persistencia D-23.2 (ADR-0012, extendido sesión 27).** `fill_zones` y
`add_zone(fill=true)` guardan el board a disco (`save_board()` interno,
incondicional en `fill_zones`; dentro del `if fill:` en `add_zone`)
inmediatamente después del bloque refill+`enforce_hole_clearance` — mismo
contrato que `route_board` desde sesión 24: al retornar OK, disco == vivo,
sin que el llamador tenga que invocar `save_board()` aparte. Si esa escritura
falla, la tool levanta `POST_ZONE_PERSIST_FAILED` (el vivo ya tiene el fix; el
disco no). `add_zone(fill=false)` no persiste — no hay nada que fillar, mismo
comportamiento que antes de sesión 27.
